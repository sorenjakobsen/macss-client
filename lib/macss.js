                ...
                if (event.shiftKey) {
                    ...
                    if (c == 48 || (c >= 65 && c <= 90)) {
                        ...
                        if (editor.__score[k]) {
                            ...
                            self.sendParams(`f 500 0 -27 -2 ${arr.join(' ')} ${editor.__ctrl['0'] || 0}`);
                            self.setHighlights(editor);
                        }
                    }
                }
...
loadScore() {
    ...
    try {
        ...
        err(score['1'].variations.length > 2, 'Error in module 1: only two variations allowed');
        err(score['2'].variations.length > 2, 'Error in module 2: only two variations allowed');
        ...
        for (var i = 0; i < 26; i++) {
            var name = String.fromCharCode(i + 97)
            var m = score[name];
            if (!m) continue;
                ...
                switch (m.type) {
                    case '*':
                        var f = function(p) {
                            ...
                            referr(s && (!score[obj.start] || score[obj.start].type != '+'), name, obj.start);
                            referr(p && (!score[obj.porta] || score[obj.porta].type != '+'), name, obj.porta);
                    ...
                    case '%':
                        var f = function(p) {
                            ...
                            referr(s && (!score[val] || score[val].type != '-'), name, val);
