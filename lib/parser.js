module.exports = (function () {
    ...
    return {
        ...
        parse: function (text) {
            ...
            if (!score['0']) {
                throw "A base rhythm ('0') must be defined.";
            }
            var voicecount = 0;
            for(var i = 0; i < score['0'].variations.length; i++) {
                if (score['0'].variations[i].divisions.length > voicecount) {
                    voicecount = score['0'].variations[i].divisions.length;
                }
            }
            if (!score['1']) {
                score['1'] = { type: '.', variations: [ Array(voicecount).fill('*'), Array(voicecount).fill('*') ] };
            }
            if (!score['2']) {
